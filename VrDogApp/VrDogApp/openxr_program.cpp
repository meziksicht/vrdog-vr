...
#pragma warning(suppress : 4996)
    ...
    bool RenderLayer(XrTime predictedDisplayTime, std::vector<XrCompositionLayerProjectionView>& projectionLayerViews,
        ...
        for (uint32_t i = 0; i < viewCountOutput; i++) {
            ...
            float aspectRatio = static_cast<float>(viewSwapchain.width) / static_cast<float>(viewSwapchain.height);
            float sY = static_cast<float>(viewSwapchain.height) / 440.0f;
            cubes[0].Scale = { sY * aspectRatio,sY,1 };
