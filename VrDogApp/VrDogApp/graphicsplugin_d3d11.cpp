...
struct D3D11GraphicsPlugin : public IGraphicsPlugin {
    ...
    void SaveFrame(const cv::Mat& img) override
    {
        if (img.empty())
            return;

        // Ensure format is 4 channels
        cv::Mat rgba;
        if (img.channels() == 4)
        {
            rgba = img;
        ...
        desc.BindFlags = D3D11_BIND_SHADER_RESOURCE; // Important!
        ...
        initData.SysMemPitch = static_cast<UINT>(rgba.step);
        ...
        hr = m_device->CreateShaderResourceView(texture, &srvDesc, &textureView);
        if (FAILED(hr))
        {
            texture->Release();
            texture = nullptr;
    ...
    void InitializeResources() {
        ...
        unsigned char* testTextureBytes = stbi_load("images.jpg", &texWidth, &texHeight,
